...
import it.seerp.Gui.Gestione.agenda.MenuAgenda;
 ...
 * @version $Id: CalendarPanel.java,v 1.9 2007/08/01 17:54:32 cknudsen Exp $
 */
public class CalendarPanel extends ObservableJPanel implements MouseWheelListener {
    ...
    // TODO: I18N
    final static String[] weekdays = {"Dom", "Lun", "Mar",
        "Mer", "Gio", "Ven", "Sab"
    };
    // TODO: I18N
    final static String[] monthNames = {"Gen", "Feb", "Mar", "Apr", "Mag",
    ...
    class MonthPanel extends JPanel implements MouseListener {
        ...
        protected DisplayedEvent getEventForPosition(int x, int y) {
            for (int i = 0; displayedEvents != null && i < displayedEvents.size(); i++) {
                ...
                if (x >= de.rect.x && x <= de.rect.x + de.rect.width && y >= de.rect.y && y <= de.rect.y + de.rect.height) {
                    return de;
                }
            }
            return null;
        ...
        public String getToolTipText(MouseEvent e) {
            ...
            if (de != null) {
                ...
                sb.append("<html>");
                if (de.event.hasTime()) {
                    sb.append(formattedTime(de.event.getHour(), de.event.getMinute()));
                    sb.append("<br>");
                }
                ...
                if (de.event.getDescription() != null && !de.event.getDescription().equals(de.event.getTitle())) {
                    sb.append("<br>");
        ...
        public void mouseClicked(MouseEvent e1) {
            ...
            for (int i = 0; displayedEvents != null && i < displayedEvents.size(); i++) {
                DisplayedEvent de = (DisplayedEvent) displayedEvents.elementAt(i);
                if (e1.getX() >= de.rect.x && e1.getX() <= de.rect.x + de.rect.width && e1.getY() >= de.rect.y && e1.getY() <= de.rect.y + de.rect.height) {
                    // Found item
            ...
            if (selectedEvent == null) {
                for (int i = 0; displayedDates != null && i < displayedDates.size(); i++) {
                    ...
                    if (e1.getX() >= dd.rect.x && e1.getX() <= dd.rect.x + dd.rect.width && e1.getY() >= dd.rect.y && e1.getY() <= dd.rect.y + dd.rect.height) {
                        // Found date
    ...
    protected void createUI() {
        ...
        JButton todayButton = new JButton("Oggi");
    ...
    private void handleResize(Graphics g) {
        ...
        this.cellHeight = (double) (this.lastHeight - this.headerHeight) / (double) NUM_WEEKS_TO_DISPLAY;
    ...
    public void paintMonth(Graphics g) {
        ...
        if (this.lastWidth != drawArea.getWidth() || this.lastHeight != drawArea.getHeight()) {
        ...
        for (int i = 0; i < 7; i++) {
            ...
            int xOffset = (int) Math.floor((this.cellWidth - (double) g.getFontMetrics(headerFont).stringWidth(text)) / (double) 2);
    ...
    protected void drawDayOfMonth(Graphics g, Calendar day,
        ...
        if (this.repository != null) {
            Vector events = this.repository.getEventInstancesForDate(day.get(Calendar.YEAR), day.get(Calendar.MONTH) + 1, day.get(Calendar.DAY_OF_MONTH));
            if (events != null) {
    ...
    public EventInstance getSelectedEvent() {
        ...
        if (this.selectedItemInd >= 0 && this.selectedItemInd < eventsForDate.size()) {
            EventInstance eventInstance = (EventInstance) eventsForDate.elementAt(this.selectedItemInd);
